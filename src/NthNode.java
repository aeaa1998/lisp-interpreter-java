...
public class NthNode extends Stack {
    int index;
    public NthNode(LinkedList<String> list) throws Exception {
        super(list);
        if(tokens.size() != 2) throw new Exception("Nth debe recibir dos argumentos");

        if (tokens.get(1) instanceof VariableReferenceStack) {
            tokens.set(1, tokens.get(1).getTokens().get(0));
        }
        if(tokens.get(1).isAtom()) throw new Exception(list.get(1) + " no es una lista");
    ...
    public String run() throws Exception {
        ...
        var list = tokens.get(1).rawValue().substring(1, holder.length() - 1).split(" ");
        if(list.length <= index) return "NILL";
        return list[index];
    }

    private iStack NthNode() throws Exception {
